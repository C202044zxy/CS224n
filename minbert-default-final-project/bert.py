...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    N, H, S, L = query.shape 
    N, H, T, L = key.shape 
    scores = torch.matmul(query, key.swapaxes(2, 3)) / math.sqrt(L) # (N, H, S, T)
    if attention_mask is not None:
      scores = scores + attention_mask 
    scores = nn.functional.softmax(scores, dim = -1) # (N, H, S, T)
    scores = self.dropout(scores) 
    output = torch.matmul(scores, value) # (N, H, S, L) 
    output = output.swapaxes(1, 2).reshape(N, S, -1) # (N, S, E) 
    return output 
...
class BertLayer(nn.Module):
  def __init__(self, config):
    ...
    self.interm_af = F.gelu # Gaussian Error Linear Unit
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    ### TODO
    # same as the implementation in transformer
    # the residual connection is before layer norm
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    x = self.add_norm(hidden_states, x, self.attention_dense, 
                      self.attention_dropout, self.attention_layer_norm) 
    # feed forward layer 
    tgt = self.interm_af(self.interm_dense(x)) 
    tgt = self.add_norm(x, tgt, self.out_dense, self.out_dropout, self.out_layer_norm) 
    return tgt 
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    ### TODO
    embedding = inputs_embeds + pos_embeds + tk_type_embeds 
    return self.embed_layer_norm(embedding) 
