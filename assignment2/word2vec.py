...
def naiveSoftmaxLossAndGradient(
    ...
    Arguments:
    Word vector length = E 
    num words in vocab = N
    ...
    y_hat = softmax(np.matmul(outsideVectors, centerWordVec)) # (N, )
    loss = - np.log(y_hat[outsideWordIdx])
    y_hat[outsideWordIdx] -= 1
    gradCenterVec = np.matmul(outsideVectors.T, y_hat) # (E, )
    gradOutsideVecs = np.outer(y_hat, centerWordVec) # (N, E)
...
def negSamplingLossAndGradient(
    ...
    p = sigmoid(np.matmul(U, centerWordVec)) # (M, )
    loss = - np.sum(np.log(1 - p)) + np.log(1 - p[0]) - np.log(p[0])
    p[0] -= 1 
    gradCenterVec = np.matmul(U.T, p) # (E, )
    gradu = np.outer(p, centerWordVec) # (M, E)
    gradOutsideVecs = np.zeros_like(outsideVectors) # (N, E) 
    np.add.at(gradOutsideVecs, indices, gradu)
    # can't use gradOutsideVecs[indices] += gradu 
    # that only add once for the variable that occur twice. 
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    outsideWordIndx = [word2Ind[x] for x in outsideWords] 
    for o in outsideWordIndx: 
        if word2vecLossAndGradient == naiveSoftmaxLossAndGradient: 
            tloss, gradc, grado = word2vecLossAndGradient(centerWordVec, o, outsideVectors, dataset) 
        else: 
            tloss, gradc, grado = word2vecLossAndGradient(centerWordVec, o, outsideVectors, dataset) 
        loss += tloss
