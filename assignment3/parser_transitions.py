...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S": # push into the stack
        ...
        if transition == "LA": # first->second, pop second
            self.dependencies.append((first, second)) 
            self.stack.append(first) 
        else: # second->first, pop first 
    ...
    def is_completed(self): 
        '''
        To judge whether the parse is finished. 
        '''
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) >= 1: 
        transitions = model.predict(unfinished_parses[:batch_size]) 
        for trans, par in zip(transitions, unfinished_parses[:batch_size]): 
            par.parse_step(trans) 
            # can NOT use par.parse(trans) 
            # because the string type that we pass into parse method will be broken down. T_T
        unfinished_parses = [x for x in unfinished_parses if not x.is_completed()] 
