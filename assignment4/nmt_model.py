...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # appoint the type of input and output. 
        # if the input type violates what we have appointed, the code will raise an error. 
        ...
        X = self.model_embeddings.source(source_padded) # (len, b, e)
        X = torch.permute(X, (1, 2, 0)) # (b, e, len)
        # the channel dimension corresponding to the embed_size
        X = torch.permute(self.post_embed_cnn(X), (2, 0, 1)) # (len, b, e) 
        enc_hiddens, (last_hidden, last_cell) = self.encoder(pack_padded_sequence(X, source_lengths)) 
        # (b, len, 2h) / (2, b, h) / (2, b, h)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens) # _ holds palce for length 
        enc_hiddens = enc_hiddens.swapaxes(0, 1) # (b, len, 2h)
        init_decoder_hidden = self.h_projection(last_hidden.swapaxes(0, 1).reshape(B, -1)) # (b, h)
        init_decoder_cell = self.c_projection(last_cell.swapaxes(0, 1).reshape(B, -1)) # (b, h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        ### YOUR CODE HERE (~9 Lines)
        ### TODO:
        ###     1. Apply the attention ptrojection layer to `enc_hiddens` to obtain `enc_hiddens_proj`,
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens) # (b, len, h) 
        Y = self.model_embeddings.target(target_padded) # (len, b, e) 
        for y in torch.split(Y, 1): 
            ybar_t = torch.cat([torch.squeeze(y, dim = 0), o_prev], dim = -1) # (b, e + h) 
            dec_state, o_prev, _ = self.step(ybar_t, dec_state, enc_hiddens, 
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_state = self.decoder(Ybar_t, dec_state) # also update state in decoder func. 
        dec_hidden, dec_cell = dec_state # each (b, h) 
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(dim = 2)) # batch matrix multiplication 
        e_t = e_t.squeeze(dim = 2)
        # (b, len, h) * (b, h, 1) -> (b, len, 1)
        ...
        alpha_t = nn.Softmax(dim = 1)(e_t) # (b, len) 
        ...
        O_t = torch.tanh(self.dropout(V_t)) # (b, h)
